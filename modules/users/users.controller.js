...
const { gravatarOptions: imageOptions } = require('../../config/appConfig');
...
const register = (req, res) => {
  ...
  User.findOne({ email: body.email }).then((user) => {
    ...
    const newUser = new User({
      name: body.name, email: body.email, password: body.password, avatar,
    });
    ...
    bcrypt.genSalt(10, (err, salt) => {
      ...
      bcrypt.hash(newUser.password, salt, (error, hash) => {
        ...
        newUser.save().then(data => res.json({ data })).catch(e => console.log('ERR::', e));
      });
    });
  });
};
...
const currentUser = (req, res) => {
  if (req.user) { return res.json({ success: true, data: req.user, message: 'User found' }); }
