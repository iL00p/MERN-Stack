...
const deletePost = (req, res) => {
  ...
  Posts.findById(id)
    .then((post) => {
      if (post.user.toString() !== user._id.toString()) { return res.status(401).json({ message: 'Not Authorized to delete this post' }); }
...
const likePost = (req, res) => {
  ...
  Posts.findById(id)
    .then((post) => {
      if (post.likes.filter(like => like.user.toString() === user._id.toString()).length > 0) { return res.status(400).json({ message: 'You have already liked this post' }); }
...
const unlikePost = (req, res) => {
  ...
  Posts.findById(id)
    .then((post) => {
      if (post.likes.filter(like => like.user.toString() === user._id.toString()).length === 0) { return res.status(400).json({ message: 'You have not yet liked this post' }); }
...
const deleteComment = (req, res) => {
  ...
  Posts.findById(postId).then((post) => {
    ...
    if (removeIndex === -1) { return res.status(404).json({ message: 'Comment not found' }); }
