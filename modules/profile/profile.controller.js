...
const setProfileFields = (body) => {
  const profileData = {};
  PROFILE_FIELDS.forEach((fieldName) => {
    if (fieldName === 'skills' && !_.isEmpty(body.skills)) { profileData.skills = body.skills.split(','); } else if (!_.isEmpty(body[fieldName])) { profileData[fieldName] = body[fieldName]; }
  });
...
const setProfileDetails = (req, res) => {
  const { user, body } = req;
...
const updateProfileCallToDb = (profileFields, res) => {
...
const getByHandle = (req, res) => {
  ...
  if (_.isEmpty(handle)) { return res.status(400).json({ message: 'Missing handle' }); }

  Profile.findOne({ handle }).populate('user', ['name', 'avatar'])
    .then((profile) => {
      if (_.isEmpty(profile)) { return res.status(404).json({ message: 'Profile not found' }); }
...
const getById = (req, res) => {
  const { params: { id } } = req;
  if (_.isEmpty(id)) { return res.status(400).json({ message: 'Missing handle' }); }
...
const getAllProfiles = (req, res) => {
  Profile.find().populate('user', ['name', 'avatar', 'email']).then((results) => {
    if (!results) { return res.status(404).json({ message: 'No profiles found' }); }
...
const addExperience = (req, res) => {
  ...
  const { errors, isValid } = Validations.validateExperience(body);
...
const deleteExperience = (req, res) => {
  ...
  if (!expId) { return res.status(400).json({ message: 'Id is required' }); }
...
const addEducation = (req, res) => {
  ...
  const { errors, isValid } = Validations.validateEducation(body);
...
const deleteEducation = (req, res) => {
  ...
  if (!eduId) { return res.status(400).json({ message: 'Id is required' }); }
...
const deleteProfile = (req, res) => {
  ...
  Profile.findOneAndRemove({ user: user._id }).then(() => {
    User.findOneAndRemove({ _id: user._id }).then(() => res.json({ success: 1, message: 'Profile successfully deleted' }));
